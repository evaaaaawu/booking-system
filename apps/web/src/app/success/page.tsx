...
export default async function SuccessPage({ searchParams }: { searchParams: Record<string, string> }) {
  ...
  return (
    <div>
      <Head>
        <title>Booking Confirmed | {eventType.title} with {user.name} | Schedule System</title>
              ...
              <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                  ...
                  <div className="mt-3 text-center sm:mt-5">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        You are scheduled in with {user.name}.
                      </p>
                    </div>
                    <div className="mt-4 border-t border-b py-4">
                      <h2 className="text-lg font-medium text-gray-600 mb-2">{eventType.title} with {user.name}</h2>
                ...
                <div className="mt-5 sm:mt-6 text-center">
                  ...
                  <div className="flex mt-2">
                    <Link href={encodeURI("https://calendar.google.com/calendar/render?action=TEMPLATE&dates=" + dayjs(date).format() + "%2F" + dayjs(date).add(eventType.length!, 'minute').format() + "&details=" + eventType.title + " with " + user.name + "&text=" + eventType.description)}>
                    ...
                    <Link href={encodeURI("https://outlook.live.com/calendar/0/deeplink/compose?body=" + eventType.description + "&enddt=" + dayjs(date).add(eventType.length!, 'minute').format() + "&path=%2Fcalendar%2Faction%2Fcompose&rru=addevent&startdt=" + dayjs(date).format() + "&subject=" + eventType.title + " with " + user.name)}>
                      <a className="mx-2 btn-wide btn-white">
                    ...
                    <Link href={encodeURI("https://outlook.office.com/calendar/0/deeplink/compose?body=" + eventType.description + "&enddt=" + dayjs(date).add(eventType.length!, 'minute').format() + "&path=%2Fcalendar%2Faction%2Fcompose&rru=addevent&startdt=" + dayjs(date).format() + "&subject=" + eventType.title + " with " + user.name)}>
                      <a className="mx-2 btn-wide btn-white">
